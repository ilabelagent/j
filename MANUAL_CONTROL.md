...
This document provides the operational runbook for the "Hydra" simulation environment. It details the manual control vectors, the analysis of specific threat scenarios, and the administration of the C2 infrastructure.
...
The system simulates the following attack vectors. Security professionals must understand these to build effective defenses.
...
4.  **Twitter "Alpha":** Bot network replying to influencers with "leaked" links.
5.  **Telegram "Support":** Fake support agents intercepting help requests.
6.  **Email Spearphishing:** "Urgent: Payroll Adjustment" emails to finance teams.
...
### Group B: Technical Exploits (11-20)
...
28. **Steganography:** Hiding payload in images.
...
31. **Frontrunning:** Monitoring mempool for victims.
32. **Ice Phishing:** Tricking user into signing "Update".
33. **Fake Airdrop:** Token with `transfer` logic that drains ETH.
34. **Address Poisoning:** Sending 0 value txs from "similar" addresses.
35. **Infinite Approval:** The classic ERC-20 drain.
36. **Malicious Seaport Order:** 0 ETH offer for NFT.
37. **L2 Bridging Trap:** Fake bridge UI.
38. **Gas Siphoning:** High gas limit on malicious contract.
39. **Sleep Minting:** Forging NFT provenance.
40. **MEV Exploits:** Sandwich attacks.

### Group E: Infrastructure (41-50)
41. **C2 over DNS:** Hiding commands in DNS TXT records.
42. **Telegram C2:** Using bots as command infrastructure.
43. **Discord C2:** Using webhooks for data exfiltration.
44. **Google Sheets C2:** Reading commands from public sheets.
...
48. **Tor Hidden Services:** .onion C2 hosting.
49. **Peer-to-Peer Botnet:** Decentralized command structure.
50. **Blockchain C2:** Storing commands in transaction data.
...
*   **Output:** `dist/payload.v[TIMESTAMP].js`
...
4.  **Action:** Victim clicks deep link -> `deeplink_factory.js` opens wallet -> `multichain.js` requests signature.
